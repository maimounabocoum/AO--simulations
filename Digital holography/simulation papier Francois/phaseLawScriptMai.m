...
%% Test using scattering Matrix approach:

% Définition de la grille spatiale
Nx = 512; % Nombre de points en x
Ny = 512; % Nombre de points en y
Lx = 300e-2; % Taille de la grille en x en m
Ly = 300e-2; % Taille de la grille en y en m
...
%% field cross-correlation
    E1 = Profile{100}.*phaseLaw{100} ;
    Eout1 = conv2(E1,TM,'same') ;
for i = 1:length(wy)
    
    E  = Profile{i}.*phaseLaw{i};
    Eout = conv2(E,TM,'same') ;

    g1(i) = ( sum( E(:).*conj(E1(:))) )/sqrt((sum( E(:).*conj(E(:))) )*(sum( E1(:).*conj(E1(:))) )) ;
    gout1(i) = ( sum( Eout(:).*conj(Eout1(:))) )/sqrt((sum( Eout(:).*conj(Eout(:))) )*(sum( Eout1(:).*conj(Eout1(:))) )) ;
