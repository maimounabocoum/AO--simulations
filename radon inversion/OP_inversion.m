...
Lobject = 1e-3;
Fc = 0.2/Lobject; % Lobject is the size of the object to detect. Using simple model (sinc function)
...
% 
FILTER = abs(MyImage.f)'*ones(1,length(MyImage.theta));
FILTER(abs(MyImage.f) >= Fc, :) = 0;

MyImage.F_R(abs(MyImage.f) >= Fc, :)  = 0;
...
img = zeros(size(x,1),size(x,2),'like',x);
  ...
  for i=1:length(MyImage.theta)
     ...
     % imagesc(MyImage.t*1e3,xsonde*1e3,t)
      imagesc(MyImage.t*1e3,xsonde*1e3,real(img))
      title(['angle',num2str(MyImage.theta(i))])
      xlabel('x')
      ylabel('y')
      xlim([0 MyImage.L*1e3])
      colorbar
      drawnow 
  end
