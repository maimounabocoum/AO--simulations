...
InfoBeam.XDebut  = -2;  InfoBeam.XFin  = -2;  InfoBeam.pasX  = .05;
InfoBeam.YDebut  = -2;  InfoBeam.YFin  = 2;     InfoBeam.pasY  = .05;
InfoBeam.ZDebut  = 36;  InfoBeam.ZFin  = 36;    InfoBeam.pasZ  = 0.5;
...
%     imagesc(toto);  colorbar; title(['champ diffracté 2D ds espace (axe(X = ' num2str(InfoBeam.XDebut + InfoBeam.pasX*(i-1)) '))']);
%     shading interp
%     pause;
%     close(gcf) 
% end
...
for i=1:InfoBeam.NbZ
    figure(i)
    toto = (squeeze(champF(:,:,i)))';
    imagesc(toto);  colorbar; title(['champ diffracté 2D ds espace (axe(Z = ' num2str(InfoBeam.ZDebut + InfoBeam.pasZ*(i-1)) '))']);
    shading interp
    pause;
    close(gcf)
end
...
%         toto = (squeeze(champF(i,:,:)))';
%         imagesc(toto);  colorbar; title(['champ diffracté 2D ds espace (axe(X = ' num2str(InfoBeam.XDebut + InfoBeam.pasX*(i-1)) '))']);
%         saveas(gcf,['PressureField2D_x_' num2str(i)], 'jpg')
%         close(gcf)
%     end
%     for i=1:InfoBeam.NbY
%         figure(i)
